...
- `ruka-shorts.html`: RukaShorts（るかしょーつ）の別ページです。TikTok / YouTube Shorts / Reels のように、検索ヒット集合または指定Clipを縦送りで視聴します。activeな1件だけTwitch iframeを生成し、`autoplay=true` / `loop=true` を付けます。スクロール / スワイプで次のClipへ進みます。検索SEO用のページではないため `noindex,follow` とし、サイトマップには含めません。
...
- RukaShorts（るかしょーつ）は `ruka-shorts.html` の別ページとして提供します。Topの「RukaShorts」、検索パネルの「ヒットを視聴」、Clipカード内のShortsボタンから入り、検索条件がある場合は検索ヒット集合を視聴プールにします。activeなClipだけTwitch iframeを生成し、`autoplay=true` / `loop=true` / `parent` を付けます。前のClipへ移る時はiframeを破棄し、スクロール / スワイプで次のClipへ進みます。
...
テストでは、PageSpeed Insightsで指摘されやすいLCP画像の優先読み込み、公開JSONのminify、主要ボタンの色コントラスト、Google Analytics 4 タグの配置とMeasurement ID、SEOキーワード拡張の人気検索リンク・JSON-LD語彙、PC版Clipモーダルの遅延iframe生成とSP版Twitchリンク維持、RukaShorts（るかしょーつ）の別ページ導線、縦スクロール、autoplay/loop付き遅延iframe生成も検証します。
