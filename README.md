...
- `ruka-shorts.html`: RukaShorts（るかしょーつ）の別ページです。TikTok / YouTube Shorts / Reels のように、検索ヒット集合または指定Clipを縦送りで視聴します。activeな1件だけTwitch iframeを生成し、`autoplay=true` / `loop=true` を付けます。PCでは16:9の横長プレイヤー、SPでは9:16の縦型プレイヤーにします。スクロール / スワイプで次のClipへ進みます。検索SEO用のページではないため `noindex,follow` とし、サイトマップには含めません。
...
- RukaShorts（るかしょーつ）は `ruka-shorts.html` の別ページとして提供します。Topの「RukaShorts」、検索パネルの「ヒットを視聴」、Clipカード内のShortsボタンから入り、検索条件がある場合は検索ヒット集合を視聴プールにします。PCでは16:9、SPでは9:16で表示し、初期12件だけDOM生成して、スクロールが進んだら8件ずつ追加します。activeなClipだけTwitch iframeを生成し、`autoplay=true` / `loop=true` / `parent` を付けます。前のClipへ移る時はiframeを破棄し、スクロール / スワイプで次のClipへ進みます。
- Clipカードのメタ情報は、ゲーム名の次行に作成者名を表示してカード内の高さを揃えます。
- 各Clipカードでは、下段に塗り/未塗りで状態を示すハートのお気に入りトグル、Shortsボタン、動画アイコンのTwitchボタン、Twitch URLコピーアイコンを `18% / 18% / 44% / 20%` の淡い連結セグメントとして表示します。各ボタンは役割別に淡く色分けし、操作アイコンは高さを揃え、コピー後はチェック表示と小さな吹き出しで知らせます。
...
テストでは、PageSpeed Insightsで指摘されやすいLCP画像の優先読み込み、公開JSONのminify、主要ボタンの色コントラスト、Google Analytics 4 タグの配置とMeasurement ID、SEOキーワード拡張の人気検索リンク・JSON-LD語彙、PC版Clipモーダルの遅延iframe生成とSP版Twitchリンク維持、RukaShorts（るかしょーつ）の別ページ導線、PC 16:9 / SP 9:16、初期12件の段階描画、縦スクロール、autoplay/loop付き遅延iframe生成も検証します。
