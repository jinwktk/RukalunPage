...
- `ruka-shorts.html`: RukaShorts（るかしょーつ）の別ページです。TikTok / YouTube Shorts / Reels のように、PC/SPとも画面いっぱいのランダム縦送りClip視聴ページとして、activeな1件だけTwitch iframeを生成します。`autoplay=true` / `muted=false` を付け、スクロール / スワイプ、Twitchの終了通知、終端付近の再生位置通知、または30秒フォールバックで次のClipへ進みます。下向きキューはPC/SP別にシークバーより上の右下寄りへ置き、次Clipが下方向にあることを示します。Top、検索、Clipカードからの導線は置かない直URL用ページです。検索SEO用のページではないため `noindex,follow` とし、サイトマップには含めません。
