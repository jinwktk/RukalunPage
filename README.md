...
Ko-fi のウィジェットはアカウント `jinnymeia` へ誘導する公開支援導線です。Ko-fi側のプロフィール、決済、表示名、入金確認などの外部アカウント操作は、このリポジトリの自動作業範囲には含めません。ページ側ではClipデータ読み込み開始後に公式scriptを動的挿入し、script失敗や描画例外が検索機能へ影響しないようguardします。PCでは既存UIに合わせた淡いピンク/ミントの「応援」ボタン、SPでは文字なしの小型追尾表示を前提にします。Ko-fi側の生成DOM/classが変わると位置補正が崩れる可能性があるため、公開前はPC/SPスクリーンショットで右下表示を確認します。
