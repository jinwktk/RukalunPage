...
- `index.html`: Clip検索ページ本体。`clip-search-data.json` を読み込み、ブラウザ内で検索・並び替え・お気に入り表示を行います。Google Analytics 4 のGoogle tagとKo-fi支援ウィジェットもここで読み込みます。
...
- おまかせボタンは、手入力検索ではその条件内から抽選し、ランダムが自動入力したClipタイトルは次回クリック時の抽選条件として再利用しません。作成者・ゲーム・並び替え条件を変更してもランダム由来の出自は保持し、候補が複数ある場合は直前Clipをできるだけ避けます。
...
- Ko-fi支援導線は公式overlay script `https://storage.ko-fi.com/cdn/scripts/overlay-widget.js` で `jinnymeia` のfloating widgetを描画します。Clipデータ読み込み開始後に動的scriptとして挿入し、SPでは文字なしの小型追尾ボタンとして右下に抑え、検索やClip操作を覆いにくくします。
...
Ko-fi のウィジェットはアカウント `jinnymeia` へ誘導する公開支援導線です。Ko-fi側のプロフィール、決済、表示名、入金確認などの外部アカウント操作は、このリポジトリの自動作業範囲には含めません。ページ側ではClipデータ読み込み開始後に公式scriptを動的挿入し、script失敗や描画例外が検索機能へ影響しないようguardします。SPでは文字なしの小型追尾表示を前提にします。Ko-fi側の生成DOM/classが変わると位置補正が崩れる可能性があるため、公開前はPC/SPスクリーンショットで右下表示を確認します。
...
テストでは、PageSpeed Insightsで指摘されやすいLCP画像の優先読み込み、公開JSONのminify、主要ボタンの色コントラスト、Google Analytics 4 タグの配置とMeasurement IDも検証します。
