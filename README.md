...
- PC版ではClipカードからモーダルウィンドウを開き、Twitch Clipをページ内で確認できます。SP版ではTwitchリンクとして開き、画面サイズと外部埋め込み負荷を避けます。中央操作は通常のリンクとして維持し、SP幅・修飾キー付きクリック・embed不可URLではブラウザ標準のリンク動作へ戻します。モーダル内のiframeはクリック時にだけ生成し、`autoplay=true` を指定します。ブラウザの自動再生ポリシーで音付き再生が止まる場合はあります。閉じるとiframeを破棄して再生と通信を止めます。モーダルはタイトルバーを持たず、グレーアウトした右上に大きな閉じるボタンを表示します。
...
- SEOキーワード拡張では、検索結果前の短い `keywordGuide` に `るっかるん`、`Rukalun`、`Twitch`、`ツイッチ`、`とぅいっち`、`Clip`、`クリップ`、`配信切り抜き`、`FF14`、`FFXIV`、`LoL`、`League of Legends`、`VALORANT`、`VALO`、`雑談`、`絶叫`、`言質`、`迷子`、`顔アイコン` を自然文として置きます。
...
GSC/GA4の確認では、GSCプロパティ `sc-domain:rukalun.mydns.jp` と GA4 property `541705085` を使います。2026-06-23時点では、GSCで `るっかるん`、`とぅいっち`、`るっかるん 顔` が少数表示され、GA4ではモバイルのエンゲージメントがデスクトップより低めだったため、`keywordGuide` を検索結果前へ移し、`とぅいっち` と `顔アイコン` を可視語彙へ追加しました。今後もGSCで実際に表示された語彙とGA4のモバイル行動を見て、短い本文と導線だけを調整します。
...
テストでは、PageSpeed Insightsで指摘されやすいLCP画像の優先読み込み、公開JSONのminify、主要ボタンの色コントラスト、Google Analytics 4 タグの配置とMeasurement ID、SEOキーワード拡張の可視本文・人気検索リンク・JSON-LD語彙、PC版Clipモーダルの遅延iframe生成とSP版Twitchリンク維持も検証します。
